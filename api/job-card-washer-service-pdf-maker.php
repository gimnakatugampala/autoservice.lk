...
<table style="width:100%;">
    ...
    <tr>
        ...
        <td align="right"><b>JOB CARD TYPE</b>: WASHER &amp; SERVICE ONLY</td>
    </tr>
</table>
...
// ── 1. Service Packages — group by unique package ID to avoid duplicate rows ──
$processedPackages = [];
...
if (!empty($data_washers)) {
    $washQty      = floatval($data_washers[0]['quantity'] ?? 0);
    $washPrice    = floatval($data_washers[0]['price']    ?? 0);
    $washDiscount = floatval($data_washers[0]['discount'] ?? 0);
    $washCode     = $data_washers[0]['washerID'] ?? '—';
    $washTotal    = ($washQty * $washPrice) - $washDiscount;
    $subTotal    += $washTotal;

    $content .= '
        <tr>
            <td>' . htmlspecialchars($washCode) . '</td>
