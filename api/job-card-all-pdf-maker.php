...
// ── Fetch real job_card_code from DB (avoids SELECT * column collision bug) ──
...
<table style="width:100%;">
    ...
    <tr>
        ...
        <td align="right"><b>NXT SERV. MILEAGE</b>: ' . htmlspecialchars($new_mileage) . ' KM</td>
    </tr>
    <tr>
    ...
    <tr>
        <td><b>CURRENT MILEAGE</b>: ' . htmlspecialchars($current_mileage) . ' KM</td>
    ...
    <tr>
        ...
        <td align="right"><b>JOB CARD TYPE</b>: ALL</td>
    </tr>
</table>
...
// ── 3. Service Packages ───────────────────────────────────────────────────────
// Group by unique ServicePackageId to avoid duplicate rows (original bug).
// Each package gets ONE row showing the combined fuel + filter total.
$processedPackages = [];

foreach ($data_fuels as $fuelItem) {
    $pkgId = $fuelItem['ServicePackageId'];
    if (!isset($processedPackages[$pkgId])) {
        $processedPackages[$pkgId] = [
            'code'        => $fuelItem['ServicePackageCode'],
            'name'        => $fuelItem['ServicePackageName'],
            'fuelTotal'   => 0.0,
            'filterTotal' => 0.0,
        ];
    }
    $processedPackages[$pkgId]['fuelTotal'] += floatval($fuelItem['price']);
}

foreach ($data_filters as $filterItem) {
    $pkgId = $filterItem['ServicePackageId'];
    if (!isset($processedPackages[$pkgId])) {
        $processedPackages[$pkgId] = [
            'code'        => $filterItem['ServicePackageCode'],
            'name'        => $filterItem['ServicePackageName'],
            'fuelTotal'   => 0.0,
            'filterTotal' => 0.0,
        ];
    }
    $processedPackages[$pkgId]['filterTotal'] += floatval($filterItem['price']);
}

foreach ($processedPackages as $pkg) {
    $pkgTotal  = $pkg['fuelTotal'] + $pkg['filterTotal'];
    $subTotal += $pkgTotal;
    $content  .= '
        <tr>
            <td>' . htmlspecialchars($pkg['code']) . '</td>
            <td class="text-uppercase">' . htmlspecialchars($pkg['name']) . '</td>
            <td>1</td>
            <td>' . number_format($pkgTotal, 2) . '</td>
            <td>0.00</td>
            <td>' . number_format($pkgTotal, 2) . '</td>
        </tr>';
}
...
if (!empty($data_washers)) {
    $washQty      = floatval($data_washers[0]['quantity']);
    $washPrice    = floatval($data_washers[0]['price']);
    $washDiscount = floatval($data_washers[0]['discount']);
    $washTotal    = ($washQty * $washPrice) - $washDiscount;
    $subTotal    += $washTotal;

    $content .= '
        <tr>
            <td>' . htmlspecialchars($data_washers[0]['washerID'] ?? '—') . '</td>
            <td class="text-uppercase">Car Wash</td>
            <td>' . $washQty . '</td>
            <td>' . number_format($washPrice,    2) . '</td>
            <td>' . number_format($washDiscount, 2) . '</td>
            <td>' . number_format($washTotal,    2) . '</td>
        </tr>';
}
